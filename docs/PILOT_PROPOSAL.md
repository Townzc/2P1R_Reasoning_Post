...
## Candidate route to exact matching (CPU feasibility now demonstrated)
...
The CPU audit below demonstrates this construction on candidate data. It does not freeze the final benchmark. Restricting problems to shared structure inventories alters the task distribution; full canonical structures must not be replaced with coarse operator labels to increase feasibility. A broader-pool sensitivity comparison remains necessary before general claims.

The CPU search is now implemented in `scripts/audit_exact_matching.py`. It samples 1024 unique four-number groups from 1..40, selects one seeded eligible target per group, and searches for 64 disjoint blocks using complete canonical structures and exact tokenizer counts. The declared first-12-structures cap limits search cost; a failed search is not an impossibility result. Candidate surface variants only change the step label (Step/Stage/Part/Line), so any match would establish token feasibility for a weak rendering control, not a sufficient semantic surface-diversity control. Outputs are candidate evidence only; no final split or training schedule is frozen by this script.
...
Before a scientific pilot, review: (1) the restricted exact-block estimand and expanded 1..40 input domain; (2) whether the narrow label-only Surface control is useful, or should be replaced by independently audited richer paraphrases; (3) group-disjoint train/development/sealed-holdout construction before augmentation; and (4) a common update schedule and decoding budget chosen using development-only evidence. The current 32-example run has zero held-out successes and cannot establish an adequate scientific training dose. These are concrete design issues, not GPU-memory blockers.

### Operator-constrained sensitivity result

Adding `--require-muldiv` selects only 64 problems from 1024 candidate groups. Expanding the candidate pool to 4096 produces the intended 256 problems in 64 blocks, with 31 complete canonical structures. Every block includes a multiply/divide structure and exact matching holds: 66416 supervised tokens per arm per 256-update cycle. Binary operator counts across 1024 paths are 636 multiplications, 348 divisions, 860 additions and 1228 subtractions. Selection retains 6.25% of candidates and selected-vs-candidate target TV is 0.22876.

This is the more relevant feasibility candidate for a four-operator task, but is still not a frozen scientific dataset. At the engineering throughput, four arms over four such cycles extrapolate to about 27.6 minutes of training alone, excluding evaluation/loading/checkpoint costs. Review of the restricted pool, richer rendering control, independent splits, training dose and exact common schedule remains necessary. See `reports/matching_operator_sensitivity.json`.
