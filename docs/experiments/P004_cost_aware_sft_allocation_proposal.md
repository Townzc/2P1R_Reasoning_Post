...
| Fixed solution teacher | [Qwen/Qwen2.5-Math-7B-Instruct](https://huggingface.co/Qwen/Qwen2.5-Math-7B-Instruct) | Candidate for bounded sampled math solutions; no download, generation, correctness yield or throughput measurement yet |
...
An offline cached teacher pool is useful for reproducibility, but its full
generation cost must be recorded as actual research expenditure. A policy may
